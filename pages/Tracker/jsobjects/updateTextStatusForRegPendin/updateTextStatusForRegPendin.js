export default {
  regPendingTextStatusToggle() {
    {
      {
        (async () => {
          let isTexted = currentRow ? currentRow.text_status : undefined;
          ...
          try {
            ...
            const result = await updateTextStatus.run();

            // Manually update the table data
            const updatedData = registrationPending.tableData.map(row => {
              if (row.id === id) {
                row.text_status = isTexted; // Update the text status for the row
              }
              return row;
            });

            // Update the table widget with the new data
            registrationPending.setData(updatedData);  // `table1` is the name of your table widget
