export default {
  orderOngoingCNRstatusToggle() {
    {
      {
        (async () => {
          let isTexted = currentRow ? currentRow.cnr_status : undefined;
          ...
          try {
            ...
            const result = await updateCNRstatus.run();

            // Manually update the table data
            const updatedData = ordersOngoing.tableData.map(row => {
              if (row.id === id) {
                row.cnr_status = isTexted; // Update the text status for the row
              }
              return row;
            });
